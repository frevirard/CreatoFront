...
export class AppDepenseComponent implements AfterViewInit {
  ...
  addRowData(row_obj: Depense): void {
  ...
  // tslint:disable-next-line - Disables all
  updateRowData(row_obj: Depense): boolean | any {
  ...
  // tslint:disable-next-line - Disables all
  deleteRowData(row_obj: Depense): boolean | any {
    this.dataSource.data = this.dataSource.data.filter((value: any) => {
...
@Component({
  ...
  templateUrl: 'depense-dialog-content.html',
  providers: [DatePipe],
})
...
export class AppEmployeeDialogContentComponent {
  ...
  constructor(
    ...
    @Optional() @Inject(MAT_DIALOG_DATA) public data: Depense
  ) {
